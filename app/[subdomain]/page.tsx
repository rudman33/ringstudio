...
export default function Page({params}:{params:Promise<{subdomain:string}>}){
  ...
  // Steps to skip for Wedding Band (no stone, shape, carat, or setting)
  const BAND_SKIP=[4,5,6,7]
  const isWeddingBand=()=>R.type==='Wedding Band'

  const next=()=>{
    if(cur===1){
      if(!contact.first.trim()){alert('Please enter your first name.');return}
      if(!contact.last.trim()){alert('Please enter your last name.');return}
      if(!emailValid(contact.email)){alert('Please enter a valid email address.');return}
      if(!phoneValid(contact.phone)){alert('Please enter a valid phone number.');return}
    }
    if(cur<T){
      let nextStep=cur+1
      if(isWeddingBand()){while(BAND_SKIP.includes(nextStep)&&nextStep<T)nextStep++}
      setCur(nextStep)
    }
  }
  const prev=()=>{
    if(cur>1){
      let prevStep=cur-1
      if(isWeddingBand()){while(BAND_SKIP.includes(prevStep)&&prevStep>1)prevStep--}
      setCur(prevStep)
    }
  }
  ...
  async function generateAiImage(){
    ...
    try{
      ...
      setAiError('Could not generate image. Please try again.')
    }
  ...
  return<div style={{minHeight:'100vh',background:'transparent'}}>
    ...
    <div style={{maxWidth:660,margin:'0 auto',padding:'1.5rem 1rem 4rem'}}>
      ...
      {cur===1&&<div><SH n={1} t="Let's start with your details" s="We'll use this to send your personalised quote."/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><div style={{marginBottom:12}}><label style={{display:'block',fontSize:11,color:INKS,textTransform:'uppercase',letterSpacing:'.07em',marginBottom:4}}>First name <span style={{color:'#C0392B'}}>*</span></label><input style={inp} value={contact.first} onChange={(e:any)=>setContact(p=>({...p,first:e.target.value}))} placeholder="Jane"/></div><div style={{marginBottom:12}}><label style={{display:'block',fontSize:11,color:INKS,textTransform:'uppercase',letterSpacing:'.07em',marginBottom:4}}>Last name <span style={{color:'#C0392B'}}>*</span></label><input style={inp} value={contact.last} onChange={(e:any)=>setContact(p=>({...p,last:e.target.value}))} placeholder="Smith"/></div></div><div style={{marginBottom:12}}><label style={{display:'block',fontSize:11,color:INKS,textTransform:'uppercase',letterSpacing:'.07em',marginBottom:4}}>Email <span style={{color:'#C0392B'}}>*</span></label><input style={inp} type="email" value={contact.email} onChange={(e:any)=>setContact(p=>({...p,email:e.target.value}))} placeholder="jane@example.com"/>{contact.email&&!emailValid(contact.email)&&<div style={{fontSize:11,color:'#C0392B',marginTop:4}}>Please enter a valid email address</div>}</div><div style={{marginBottom:12}}><label style={{display:'block',fontSize:11,color:INKS,textTransform:'uppercase',letterSpacing:'.07em',marginBottom:4}}>Phone <span style={{color:'#C0392B'}}>*</span></label><input style={inp} type="tel" value={contact.phone} onChange={(e:any)=>{const v=e.target.value.replace(/[^\d\s()+-]/g,'');setContact(p=>({...p,phone:v}))}} placeholder="+1 (416) 555-0100"/>{contact.phone&&!phoneValid(contact.phone)&&<div style={{fontSize:11,color:'#C0392B',marginTop:4}}>Please enter a valid phone number</div>}</div><Nav dis={!contact.first.trim()||!contact.last.trim()||!emailValid(contact.email)||!phoneValid(contact.phone)}/></div>}
      ...
      {cur===3&&<div><SH n={3} t="Have an inspiration photo?" s="Upload a picture or continue from scratch."/><input ref={fileInputRef} type="file" accept="image/*" onChange={handlePhotoSelect} style={{display:'none'}}/><div onClick={()=>!photoUploading&&fileInputRef.current?.click()} style={{border:'1px dashed '+BDRS,borderRadius:12,padding:R.photo&&R.photo!=='none'?'1rem':'2.5rem 2rem',textAlign:'center',background:W,cursor:photoUploading?'wait':'pointer',opacity:photoUploading?.7:1}}>{photoUploading?<div style={{fontSize:14,fontWeight:500,color:INKS}}>Uploading…</div>:R.photo&&R.photo!=='none'?<img src={R.photo} alt="Inspiration" style={{maxWidth:'100%',maxHeight:240,borderRadius:8,objectFit:'contain'}}/>:<><div style={{fontSize:32,marginBottom:10}}>📷</div><div style={{fontSize:14,fontWeight:500,color:INKS}}>Click to upload a photo</div></>}</div><div style={{fontSize:11,color:INKG,textAlign:'center',marginTop:8}}>JPG, PNG or WEBP · Max 5MB</div>
      {(!R.photo||R.photo==='none')&&<><div style={{display:'flex',alignItems:'center',gap:12,margin:'1rem 0'}}><div style={{flex:1,height:1,background:BDR}}/><span style={{fontSize:11,color:INKG}}>OR</span><div style={{flex:1,height:1,background:BDR}}/></div><button onClick={()=>{set('photo','none');next()}} style={{width:'100%',background:W,border:'1px solid '+BDRS,borderRadius:8,padding:12,fontSize:14,color:INKS,cursor:'pointer',fontFamily:'inherit'}}>🪄 Continue with the wizard</button></>}
      <div style={{marginTop:'1.5rem'}}><button style={bb} onClick={prev}>← Back</button>{R.photo&&R.photo!=='none'&&<button onClick={()=>setCur(4)} style={{...bn,marginLeft:'auto'}}>Next →</button>}</div></div>}
